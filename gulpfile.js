...
gulp.task('ship-to-gitlab', function (done) {
    ...
    var gitPath = 'https://' + user + ':' + token + `@gitlab.syncfusion.com/content/xamarin.android-docs`;
    console.log('Clone has been started...!');
    var clone = shelljs.exec('git clone ' + gitPath + ' -b ' + branch + ' ' + `../../../gitlabRepo/xamarin.android-docs`, {
        silent: false
    });
    if (clone.code !== 0) {
        ...
        for (var changedFileName of changedFileNames.split(',')) {
            ...
            if (changedFileName !== null && changedFileName !== '' && changedFileName !== '.gitignore') {
                ...
                if (fs.existsSync('../xamarin.android-docs/' + changedFileName)) {
                    // It will update the modified files
                    if (fs.existsSync('../../../gitlabRepo/xamarin.android-docs/' + changedFileName)) {
                        shelljs.cp('-rf', `../xamarin.android-docs/` + changedFileName, `../../../gitlabRepo/xamarin.android-docs/` + changedFileName);
                    }
                    else {
                        // It will update the newly added files
                        if (fs.existsSync('../../../gitlabRepo/xamarin.android-docs/')) {
                            shelljs.cp('-rf', `../xamarin.android-docs/` + changedFileName, `../../../gitlabRepo/xamarin.android-docs/` + changedFileName);
                        }
                    }
                    ...
                    if (fs.existsSync('../../../gitlabRepo/xamarin.android-docs/' + changedFileName)) {
                        shelljs.rm('-rf', `../../../gitlabRepo/xamarin.android-docs/` + changedFileName);
        ...
        shelljs.cd(`../../../gitlabRepo/xamarin.android-docs`);
